        ...
        {!isPaywall && (
          ...
          >
            <div className="flex flex-1 flex-col">
              <div
                className={cn(
                  "flex shrink-0 items-center justify-center border-b border-white/[0.04] py-1",
              ...
              <nav className="flex flex-1 flex-col space-y-px px-2 py-2" aria-label="App">
                {primaryNavLinks.map((link) => (
                  <Link
                    ...
                    title={link.hint}
                    className={cn(
                      "flex items-center gap-2.5 rounded-md py-[7px] text-[13px] font-medium tracking-tight transition-colors",
                      sidebarCollapsed ? "justify-center px-0" : "border-l border-transparent pl-2.5 pr-2",
                      isActive(link.href)
                        ? sidebarCollapsed
                          ? "bg-white/[0.08] text-text"
                          : "border-l-white/20 bg-white/[0.035] text-text"
                        : sidebarCollapsed
                          ? "text-text-3 hover:bg-white/[0.025] hover:text-text-2"
                          : "text-text-3 hover:bg-white/[0.025] hover:text-text-2"
                    )}
                  >
                    <NavGlyph
                      name={link.icon}
                      className={cn(isActive(link.href) ? "text-text" : "text-text-3")}
                    />
                ...
                {!sidebarCollapsed && (
                  <div className="mt-2.5 border-t border-white/[0.04] pt-2" role="group" aria-label="Coming soon">
                    <p className="px-2.5 pb-1 text-[10px] font-medium uppercase tracking-[0.14em] text-text-3/70">
                      Soon
                    </p>
                    <div className="space-y-px">
                      {COMING_SOON_LINKS.map((item) => (
                        <div
                          key={item.label}
                          className="flex cursor-not-allowed items-center gap-2.5 rounded-md py-[7px] pl-2.5 pr-2 text-[12px] font-medium text-text-3/50"
                          aria-disabled="true"

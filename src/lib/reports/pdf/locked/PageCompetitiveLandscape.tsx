...
function shareAsideParagraph(data: ReportData, slices: readonly ShareSlice[]): string {
  const clientDisplay =
  ...
  return clipPdfText(
    `${clientDisplay} accounts for roughly ${pct}% of AI recommendations in this category. Competitors occupy the remaining share — meaning buyers are consistently shown multiple comparable options.`,
    520,
  );
}
...
export function PageCompetitiveLandscape({ data }: { data: ReportData }): ReactElement {
  ...
  const aside = shareAsideParagraph(data, shareSlices);
    ...
    <PdfInnerPage title={LOCKED_PAGE_HEADER[4]!}>
      ...
      <View style={lockedStyles.comp_pieRow} wrap={false}>
        <View style={lockedStyles.comp_pieColChart} wrap={false}>
          <Text style={lockedStyles.comp_pieChartTitle}>{"Share of AI Recommendations"}</Text>
        ...
        <View style={lockedStyles.comp_pieColAside} wrap={false}>
          <Text style={lockedStyles.comp_pieAside}>{aside}</Text>
        </View>
      </View>
